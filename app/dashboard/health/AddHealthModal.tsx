...
export default function AddHealthModal({ cows }: { cows: Cow[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { error: err } = await supabase.from('health_log').insert({
      ...
      if (err) throw err
      setOpen(false)
      setForm({ cow_local_id: cows[0]?.local_id ?? '', date: today, event_type: 'Vaccination', description: '', drug_used: '', dosage: '', vet_name: '', cost: '', next_due_date: '' })
      router.refresh()
    } catch (err: unknown) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between">
              <h2 className="text-lg font-bold text-moka-900">Log Health Event</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <Field label="Description">
                <textarea className={inp} rows={2} value={form.description} onChange={e => set('description', e.target.value)} placeholder="What happened?" />
              </Field>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Drug Used">
                  <input className={inp} value={form.drug_used} onChange={e => set('drug_used', e.target.value)} placeholder="optional" />
                </Field>
                <Field label="Dosage">
                  <input className={inp} value={form.dosage} onChange={e => set('dosage', e.target.value)} placeholder="e.g. 5ml" />
                </Field>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Vet Name">
                  <input className={inp} value={form.vet_name} onChange={e => set('vet_name', e.target.value)} placeholder="optional" />
                </Field>
              ...
              <Field label="Next Due Date">
                <input type="date" className={inp} value={form.next_due_date} onChange={e => set('next_due_date', e.target.value)} />
              </Field>
